...
def compareme(dir1, dir2):       #递归获取更新项函数
	...
	#将更新文件名或者目录追加的holderlist
	[holderlist.append(os.path.abspath(os.path.join(dir1,x))) for x in only_in_one]
	[holderlist.append(os.path.abspath(os.path.join(dir1,x))) for x in diff_in_one]
	if len(dircomp.common_dirs) > 0:  #判断是否存在相同的目录，以便递归
		for item in dircomp.common_dirs:  #递归子目录
			compareme(os.path.abspath(ops.path.join(dir1,item)), \
...
def main():
	...
	for item in source_files:  #遍历返回的差异文件或目录清单
		destination_dir=re.sub(dir1, dir2, item)  #将源目录差异路径清单对比替换成备份目录
		
		destination_files.append(destination_dir)
		if os.path.isdir(item): #如果差异路径目为目录切不存在，则在备份目录中新建
			if not os.path.exits(destination_dir):
